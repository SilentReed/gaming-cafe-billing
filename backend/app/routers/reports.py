...
@router.get("/daily")
def daily_report(date: str = Query(None), db: DBSession = Depends(get_db)):
    if not date:
...
@router.get("/range")
def range_report(start: str, end: str, db: DBSession = Depends(get_db)):
    rows = db.execute(
        sa.text(
            "SELECT date(ended_at) as d, COUNT(*), COALESCE(SUM(final_amount), 0), COALESCE(SUM(bonus_amount), 0), COALESCE(SUM(duration_min), 0) "
            "FROM bills WHERE date(ended_at) BETWEEN :start AND :end AND status != 'refunded' "
...
def console_utilization(start: str | None = None, end: str | None = None, db: DBSession = Depends(get_db)):
...
@router.get("/export")
def export_bills(start_date: str | None = None, end_date: str | None = None, token: str | None = Query(None), db: DBSession = Depends(get_db)):
...
def member_activity(start: str | None = None, end: str | None = None, db: DBSession = Depends(get_db)):
    query = (
